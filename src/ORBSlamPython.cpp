...
BOOST_PYTHON_MODULE(orbslam2)
{
    ...
    boost::python::class_<ORBSlamPython, boost::noncopyable>("System", boost::python::init<const char *, const char *, boost::python::optional<ORB_SLAM2::System::eSensor>>())
        ...
        .def("set_keyframe_interval", &ORBSlamPython::setKeyFrameInterval)
        .def("load_and_process_stereo", &ORBSlamPython::loadAndProcessStereo)
        .def("track_with_icp_pose", &ORBSlamPython::trackWithIcpPose)
        .def("track_with_orb_feature", &ORBSlamPython::trackWithOrbFeature)
...
void ORBSlamPython::setKeyFrameInterval(int interval)
{
    if (system)
    {
        ;
...
bool ORBSlamPython::trackWithOrbFeature(cv::Mat image, cv::Mat depthImage, double timestamp)
{
    if (!system)
    {
        return false;
...
bool ORBSlamPython::trackWithIcpPose(cv::Mat image, cv::Mat depthImage, cv::Mat IcpPose_t1_t0, double timestamp)
{
    if (!system)
    {
        return false;
    ...
    if (image.data && depthImage.data)
    {
        system->icpMixMode = true;
        cv::Mat pose = system->TrackRGBDwithIcpInit(image, depthImage, IcpPose_t1_t0, timestamp);
        // cv::Mat pose = system->TrackRGBD(image, depthImage, timestamp);
        return !pose.empty();
    }
    else
...
boost::python::list ORBSlamPython::getTrajectoryPoints() const
{
    ...
    int frame_id = 0;
    for (std::list<cv::Mat>::iterator lit = system->GetTracker()->mlRelativeFramePoses.begin(), lend = system->GetTracker()->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++, frame_id++)
    {
